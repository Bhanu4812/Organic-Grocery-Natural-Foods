(() => {
  ...
  $$('.direction-toggle').forEach((button) => button.addEventListener('click', () => {
  ...
  if (menu) {
    ...
    $$('.nav-list > li:not(.mobile-extra) > .nav-link', menu).forEach((link) => {
      ...
      link.insertAdjacentHTML('afterbegin', `<i class="mobile-nav-icon" data-lucide="${menuIcons[page] || 'circle'}"></i>`);
    });
  ...
  $$('.search-toggle').forEach((button) => button.addEventListener('click', () => {
  ...
  const productId = (button) => (button.dataset.product || 'organic-product')
  ...
  const updateCartUI = () => {
    ...
    $$('.add-cart').forEach((button) => {
      ...
      button.innerHTML = added ? '<i data-lucide="check"></i><span>Added to Cart</span>' : '<i data-lucide="shopping-bag"></i><span>Add to Cart</span>';
    });
    drawIcons();
  };
  const toast = $('.toast');
  $$('.add-cart').forEach((button) => button.addEventListener('click', () => {
    ...
    if (adding) cartItems.push(id); else cartItems.splice(existing, 1);
  ...
  $$('[data-qty]').forEach((wrapper) => {
    ...
    $$('button', wrapper).forEach((button) => button.addEventListener('click', () => {
      input.value = Math.max(1, Number(input.value) + (button.dataset.action === 'plus' ? 1 : -1));
    }));
  });
  ...
  const validatePasswordMatch = () => {
    ...
    confirmPassword.setCustomValidity(confirmPassword.value && confirmPassword.value !== password ? 'Passwords do not match.' : '');
  };
  confirmPassword?.addEventListener('input', validatePasswordMatch);
  $('#password')?.addEventListener('input', validatePasswordMatch);
  $$('form[data-validate]').forEach((form) => form.addEventListener('submit', (event) => {
    ...
    if (!valid) event.preventDefault();
      ...
      if (toast) {
        toast.textContent = form.dataset.contactForm !== undefined ? 'Thanks! Your enquiry has been received.' : 'Thanks! Your request has been received.';
        toast.classList.add('show');
  ...
  $$('[role=tab]').forEach((tab) => tab.addEventListener('click', () => {
    ...
    $$('[role=tabpanel]', tabs).forEach((panel) => { panel.hidden = true; });
    tab.setAttribute('aria-selected', 'true');
    document.getElementById(tab.getAttribute('aria-controls')).hidden = false;
  }));
  const filter = (selector, attribute) => $$(selector).forEach((control) => control.addEventListener('change', () => {
    const value = control.value.toLowerCase();
    $$(`[data-${attribute}]`).forEach((item) => {
      item.hidden = Boolean(value && value !== 'all' && !item.dataset[attribute].toLowerCase().includes(value));
    });
  }));
